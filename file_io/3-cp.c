...
int main(int argc, char *argv[])
{
	...
	if (fd_from == -1)
		return (print_error("Can't read from file", 98, fd_from, fd_to));

	fd_to = open(file_to, O_WRONLY | O_CREAT | O_TRUNC,
			S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
	if (fd_to == -1)
		return (print_error("Can't write to file", 99, fd_from, fd_to));
	...
	if (close(fd_from) == -1 || close(fd_to) == -1)
		return (print_error("Can't close fd", 100, fd_from, fd_to));
